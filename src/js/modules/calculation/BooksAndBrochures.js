...
export class BooksAndBrochures {
  constructor() {
    ...
    document
      .getElementById(pagesInner)
      .addEventListener('change', this.getProductPrice.bind(this))
    document
      .getElementById(pagesBlack)
      .addEventListener('change', this.getProductPrice.bind(this))
    document
      .getElementById(pagesColor)
      .addEventListener('change', this.getProductPrice.bind(this))
    document
      .getElementById(pagesEmpty)
      .addEventListener('change', this.getProductPrice.bind(this))
    document
      .getElementById(sheetsInner)
      .addEventListener('change', this.getProductPrice.bind(this))
    document
    ...
    document
      ...
      .addEventListener('change', this.getProductPrice.bind(this))
    document
      .getElementById(printCover)
      .addEventListener('change', this.getProductPrice.bind(this))
    document
      .getElementById(sheetsCover)
      .addEventListener('change', this.getProductPrice.bind(this))
    document
      .getElementById(foil)
      .addEventListener('change', this.getProductPrice.bind(this))
    document
      .getElementById(binding)
      .addEventListener('change', this.getProductPrice.bind(this))
    document
      .getElementById(wings)
      .addEventListener('click', this.getProductPrice.bind(this))
    this.resetValues()
  ...
  getProductPrice() {
    ...
    const sizeProd = getService(
      this.variables.size,
      this.theFormProduct,
      priceList.productSize
    )
    const materialProd = getService(
      this.variables.material,
      this.theFormProduct,
      priceList.productMaterial
    )
    const coverMaterial = getService(
      this.variables.cover,
      this.theFormProduct,
      priceList.productCover
    )
    const sheetsForCover = getService(
      this.variables.sheetsCover,
      this.theFormProduct,
      priceList.productSheets
    )
    const sheetsForInner = getService(
      this.variables.sheetsInner,
      this.theFormProduct,
      priceList.productSheets
    )
    const binding = getService(
      this.variables.binding,
      this.theFormProduct,
      priceList.productBinding
    )
    ...
    const wings = getWings(
      ...
      foilForCover,
      this.theFormProduct,
      this.variables.wings
    )
    const printInner = this.printInner.getPriceInnerPrint(
      count,
      sizeProd,
      materialProd,
      sheetsForInner
    ...
    const sumOfvalues =
      (sizeProd * 2 * coverMaterial * sheetsForCover + printInner + binding) *
        count + foilForCover + printForCover + wings
    //----------------------------------------------------------------------//
  ...
  resetValues() {
    ...
    reset.forEach((res) =>
      res.addEventListener(
        'click',
        () => (this.result.textContent = result.toFixed(2))
      )
    )
  }
}
